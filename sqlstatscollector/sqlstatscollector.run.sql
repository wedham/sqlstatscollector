...
RAISERROR(N'/****** Object:  StoredProcedure [collect].[run] ******/', 10, 1) WITH NOWAIT
GO
...
IF OBJECT_ID(N'[collect].[run]', N'P') IS NULL
BEGIN
	EXEC ('CREATE PROCEDURE [collect].[run] AS SELECT NULL')
END
GO
...
2024-01-17  Mikael Wedham		+Adapted to CRON v2
*******************************************************************************/
ALTER PROCEDURE [collect].[run]
AS
BEGIN
